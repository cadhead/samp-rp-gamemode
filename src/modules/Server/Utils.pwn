...
DeserializePosition(const pos[], va_args<>) {
  sscanf(pos, "p<|>fffii", va_start<1>);
}

SerializePosition(dest[], size = sizeof dest, va_args<>) {
  format(dest, size, "%.3f|%.3f|%.3f|%d|%d", va_start<2>);
}
